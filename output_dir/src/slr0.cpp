...
#pragma HLS inline off
    for (int i = 0; i < 400; i++){
    #pragma HLS pipeline II=1
float1 ve1_off = ve1[i];
ap_axiu<32, 0, 0, 0> ve1_on;
...
#pragma HLS inline off
    for (int i = 0; i < 400; i++){
    #pragma HLS pipeline II=1
float1 ve2_off = ve2[i];
ap_axiu<32, 0, 0, 0> ve2_on;
...
#pragma HLS inline off
    for (int i = 0; i < 400; i++){
    #pragma HLS pipeline II=1
float1 vu1_off = vu1[i];
ap_axiu<32, 0, 0, 0> vu1_on;
...
#pragma HLS inline off
    for (int i = 0; i < 400; i++){
    #pragma HLS pipeline II=1
float1 vu2_off = vu2[i];
ap_axiu<32, 0, 0, 0> vu2_on;
...
#pragma HLS inline off
    ...
    #pragma HLS pipeline II=1
float1 vx_off;
ap_axiu<32, 0, 0, 0> vx_on = fifo_x_to_off_chip.read();
uint32_t vx_tmp[1];
vx_tmp[0] = vx_on.data.range(31,0);
vx_off[0] = *(float*)(&vx_tmp[0]);vx[i] = vx_off;
    }
}
...
#pragma HLS inline off
    ...
    #pragma HLS pipeline II=1
float1 vw_off;
ap_axiu<32, 0, 0, 0> vw_on = fifo_w_to_off_chip.read();
uint32_t vw_tmp[1];
vw_tmp[0] = vw_on.data.range(31,0);
vw_off[0] = *(float*)(&vw_tmp[0]);vw[i] = vw_off;
    }
}
...
#pragma HLS inline off
    ...
    #pragma HLS pipeline II=1
float1 vA_off;
ap_axiu<32, 0, 0, 0> vA_on = fifo_A_to_off_chip.read();
uint32_t vA_tmp[1];
vA_tmp[0] = vA_on.data.range(31,0);
vA_off[0] = *(float*)(&vA_tmp[0]);vA[i] = vA_off;
    }
}
...
return;
    ...
    for (int d0 = 0; d0 < 400; d0++){
        for (int d1 = 0; d1 < 400; d1+=1){
        #pragma HLS pipeline II=1
ap_axiu<32, 0, 0, 0> tmp_fifo = fifo_A_from_off_chip_to_S0.read();
...
#pragma HLS inline off
    for (int d0 = 0; d0 < 400; d0+=1){
    #pragma HLS pipeline II=1
ap_axiu<32, 0, 0, 0> tmp_fifo = fifo_y_from_off_chip_to_S1.read();
uint32_t tmp_0 =  tmp_fifo.data.range(31,0);
...
return;
    ...
    for (int d0 = 0; d0 < 400; d0+=1){
    #pragma HLS pipeline II=1
ap_axiu<32, 0, 0, 0> tmp_fifo = fifo_x_from_off_chip_to_S1.read();
uint32_t tmp_0 =  tmp_fifo.data.range(31,0);
...
int d0 = d0_0 * 0 + d0_1;
ap_axiu<32, 0, 0, 0> tmp_fifo = fifo_x_from_task2_to_task3.read();
if (d0 + 0 < 400)
{
uint32_t tmp_0 =  tmp_fifo.data.range(31,0);
    x[d0 + 0 + 0]  = *(float*)(&tmp_0);
...
return;
    ...
    for (int d0 = 0; d0 < 400; d0+=1){
    #pragma HLS pipeline II=1
ap_axiu<32, 0, 0, 0> tmp_fifo = fifo_w_from_off_chip_to_S3.read();
uint32_t tmp_0 =  tmp_fifo.data.range(31,0);
